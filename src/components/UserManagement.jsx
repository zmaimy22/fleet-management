...
export default function UserManagement() {
  ...
  useEffect(() => {
    loadAndInitializeUsers();
  }, []);

  const loadAndInitializeUsers = async () => {
    try {
      setLoading(true);
      ...
      if (saved) {
        try {
          ...
          // No saved users, initialize defaults
          await initializeDefaultUsers();
        }
      } else {
        // No saved users anywhere, initialize defaults
        await initializeDefaultUsers();
      }
    } finally {
      setLoading(false);
  ...
  const updateUsersList = async (newUsers) => {
    setUsers(newUsers);
    try {
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <div className="space-y-4">
              ...
              {editingUser.role === 'driver' && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Vincular a Conductor *</label>
                  <select
