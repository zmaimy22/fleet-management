...
const Stats = ({ drivers, schedule }) => {
  ...
  // Calculate work days, vacations, and sick leaves from schedule
  let totalWorkDays = 0;
  let totalVacations = 0;
  let totalSickLeaves = 0;
  
  if (schedule) {
    drivers.filter(d => d.type === 'driver').forEach(driver => {
      const driverSchedule = schedule[driver.id];
      if (driverSchedule) {
        Object.values(driverSchedule).forEach(day => {
          if (day?.type === 'work') totalWorkDays++;
          if (day?.type === 'vacation') totalVacations++;
          if (day?.type === 'sick' && day?.value !== 'HS') totalSickLeaves++;
        });
      }
    });
  }
  
  // Count active routes (routes that are used by drivers)
  const activeRoutes = new Set();
  drivers.filter(d => d.type === 'driver').forEach(driver => {
    if (driver.route) {
      const routes = driver.route.split('/');
      routes.forEach(r => activeRoutes.add(r.trim()));
    }
    if (driver.assignedRoute) activeRoutes.add(driver.assignedRoute);
    if (driver.availableRoutes) {
      driver.availableRoutes.forEach(r => activeRoutes.add(r));
    }
  });
