...
const CellEditModal = ({ isOpen, onClose, driver, day, cellData, onSave }) => {
  ...
  useEffect(() => {
    if (cellData) {
      setType(cellData.value === 'HS' ? 'union' : cellData.type);
  ...
  const handleSave = () => {
    const finalType = type === 'union' ? 'work' : type;
    const finalValue = type === 'union' ? 'HS' : value;
    onSave(driver, day, { type: finalType, value: finalValue });
    onClose();
  };
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-50 flex items-center gap-2">
              {driver?.originalCategory === 'lanzarote' && '😢 Rutas a Lanzarote'}
              {driver?.originalCategory === 'local_morning' && '☀️ Rutas Locales Mañana'}
              {driver?.originalCategory === 'local_night' && '🌙 Rutas Locales Noche'}
              {driver?.originalCategory === 'supervisors' && '👔 Encargados / Jefe de Logística'}
              {driver?.originalCategory === 'loaders' && '📦 Cargadores'}
              {!driver?.originalCategory && '👥 Otros'}
              {cellData?.type === 'sick' && cellData?.value !== 'HS' && (
                <span className="ml-auto px-2 py-1 bg-red-100 text-red-700 text-xs font-bold rounded-full border border-red-300">
                  🏥 En Baja Hoy
                </span>
              )}
              {cellData?.value === 'HS' && (
                <span className="ml-auto px-2 py-1 bg-cyan-100 text-cyan-700 text-xs font-bold rounded-full border border-cyan-300">
                  🏛️ Horas Sindical
                </span>
              )}
            </div>
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setType(nt);
                if (nt === 'union') { setCustomValue(false); setValue('HS'); return; }
                if (nt !== 'work') setCustomValue(false);
                if (nt === 'weekend') setValue('');
                else if (nt === 'vacation') setValue('V');
                else if (nt === 'sick' && value !== 'HS') setValue('Baja');
              }}
            ...
            >
              ...
              <option value="union">Horas Sindical (HS)</option>
            </select>
          </div>
          ...
          <div>
            ...
            {type === 'union' ? (
              <div className="w-full px-4 py-2 border border-gray-300 rounded-lg bg-cyan-50 text-cyan-700 font-semibold">HS - Horas Sindical</div>
              ...
              driver?.type === 'loader' ? (
                ...
                >
                  ...
                  <option value="CM">CM - Mañana</option>
                  <option value="CT">CT - Tarde</option>
                </select>
              ) : driver?.type === 'supervisor' ? (
                <>
                  <select
                    value={value === 'P' || value === 'GT' ? value : 'Ruta'}
                    onChange={(e) => {
                  ...
                  >
                    <option value="">Seleccionar</option>
                    <option value="P">P - En Planta</option>
                    <option value="GT">GT - Guardia Tarde</option>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {driver?.type === 'loader' && (
                <>
                  ...
                  <button
                    onClick={() => {setType('union'); setValue('HS');}}
                    className="px-3 py-1 bg-teal-500 text-white text-xs rounded hover:bg-teal-600 transition"
                  >
                    HS
                  </button>
                </>
